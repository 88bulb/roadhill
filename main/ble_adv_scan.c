...
void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
  ...
  case ESP_GAP_BLE_SCAN_RESULT_EVT: {
    /*
       ...
       *)param; switch (scan_result->scan_rst.search_evt) { case
       ESP_GAP_SEARCH_INQ_RES_EVT: { mfr_data = esp_ble_resolve_adv_data(
                ...
                if (mfr_data_len > 0) {
                    ...
                    if (!is_bulbcast(mfr_data) && !is_testcast(mfr_data))
                        return;
                    char *bda_str = u8_to_hex(scan_result->scan_rst.bda, 6,
       bda_buf, sizeof(bda_buf)); char *mfr_str = u8_to_hex(mfr_data,
       mfr_data_len, mfr_buf, sizeof(mfr_buf)); if (is_bulbcast(mfr_data)) {
                        printf("bulbcast %s %s\n", bda_str, mfr_str);
...
void ble_adv_scan(void *args) {
  ...
  */
  ...
  while (1) {
    ...
    xEventGroupWaitBits(ble_events, ADV_START_COMPLETE, pdFALSE, pdFALSE, 0);
