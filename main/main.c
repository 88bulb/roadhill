  ...
  if (0 == strcmp(cmd, "OTA")) {
    ...
    if (tracks_array_size) {
      ...
      for (int i = 0; i < p->tracks_array_size; i++) {
        ...
        int position_ms = cJSON_GetObjectItem(item, "position")->valueint;
...
void app_main(void) {
  ...
  strlcpy((char *)sta_cfg.sta.ssid, (char *)ap.ssid, sizeof(sta_cfg.sta.ssid));
  ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &sta_cfg));

  wifi_config_t ap_cfg = ap_config_default;
  strlcpy((char *)ap_cfg.ap.ssid, "juwanke-soundbar-", sizeof(ap_cfg.ap.ssid));
