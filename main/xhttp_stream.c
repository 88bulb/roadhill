...
#include <sys/stat.h>
#include <string.h>
#include <strings.h>    // BSD flavored
...
static const char *TAG = "XHTTP_STREAM";
#define MAX_PLAYLIST_LINE_SIZE (512)
// #define HTTP_STREAM_BUFFER_SIZE (2048)
#define HTTP_STREAM_BUFFER_SIZE (256 * 1024)

typedef struct xhttp_stream {
    audio_stream_type_t type;
    bool is_open;
    esp_http_client_handle_t client;
    xhttp_stream_event_handle_t hook;
...
static esp_err_t xhttp_stream_auto_connect_next_track(audio_element_handle_t el);
...
static esp_codec_type_t get_audio_type(const char *content_type) {
    ...
    if (strncasecmp(content_type, "audio/x-scpls", strlen("audio/x-scpls")) ==
        0) {
...
static int dispatch_hook(audio_element_handle_t self,
                         xhttp_stream_event_id_t type, void *buffer,
                         int buffer_len) {
    xhttp_stream_t *xhttp_stream = (xhttp_stream_t *)audio_element_getdata(self);

    xhttp_stream_event_msg_t msg;
    msg.event_id = type;
    msg.http_client = xhttp_stream->client;
    msg.user_data = xhttp_stream->user_data;
    msg.buffer = buffer;
    msg.buffer_len = buffer_len;
    msg.el = self;
    if (xhttp_stream->hook) {
        return xhttp_stream->hook(&msg);
    }
    return ESP_OK;
}

static bool _is_playlist(audio_element_info_t *info, const char *uri) {
    if (info->codec_fmt == ESP_AUDIO_TYPE_M3U8 ||
        info->codec_fmt == ESP_AUDIO_TYPE_PLS) {
        return true;
    }
    char *dot = strrchr(uri, '.');
    if (dot &&
        (strcasecmp(dot, ".m3u") == 0 || strcasecmp(dot, ".m3u8") == 0)) {
        return true;
    }
    return false;
}

static bool _get_line_in_buffer(xhttp_stream_t *http, char **out) {
    *out = NULL;
    char c;
...
static char *_client_read_line(xhttp_stream_t *http) {
    ...
    if (need_read > 0) {
        if (http->playlist->remain > 0) {
            memmove(http->playlist->data,
                    http->playlist->data + http->playlist->index,
                    http->playlist->remain);
            http->playlist->index = 0;
        }
        rlen = esp_http_client_read(
            http->client, http->playlist->data + http->playlist->remain,
            need_read);
        if (rlen > 0) {
...
static esp_err_t _resolve_playlist(audio_element_handle_t self,
                                   const char *uri) {
    audio_element_info_t info;
    xhttp_stream_t *http = (xhttp_stream_t *)audio_element_getdata(self);
    ...
    if (info.codec_fmt == ESP_AUDIO_TYPE_PLS) {
        ...
        while ((line = _client_read_line(http))) {
            ...
            if (!strncmp(line, "File",
                         sizeof("File") - 1)) { // this line contains url
                int i = 4;
                while (line[i++] != '=')
                    ; // Skip till '='
...
#define VARIANT_TAG "#EXT-X-STREAM-INF"
    ...
    while ((line = _client_read_line(http))) {
        ...
        if (!is_playlist_uri && strstr(line, "#EXTINF") == (void *)line) {
            ...
            continue;
        } else if (!is_playlist_uri &&
                   strstr(line, VARIANT_TAG) == (void *)line) {
            /**
             * Non-standard attribute.
             * There are multiple variants of audios. We do not support this for
             * now.
             */
            is_playlist_uri = true;
            continue;
...
_stream_open_begin:
    ...
    if (uri == NULL) {
        if (http->is_playlist_resolved && http->enable_playlist_parser) {
            if (dispatch_hook(self, HTTP_STREAM_FINISH_PLAYLIST, NULL, 0) !=
                ESP_OK) {
    ...
    if (dispatch_hook(self, XHTTP_STREAM_PRE_REQUEST, NULL, 0) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to process user callback");
        return ESP_FAIL;
    }
...
_stream_redirect:
    ...
    /*
     * Due to the total byte of content has been changed after seek, set
     * info.total_bytes at beginning only.
     */
    ...
    if (_is_playlist(&info, uri) == true) {
        /**
         * `goto _stream_open_begin` blocks on http_open until it gets valid
         * URL. Ensure that the stop command is processed
         */
...
static int _http_read(audio_element_handle_t self, char *buffer, int len,
    ...
    if (rlen <= 0 && http->auto_connect_next_track) {
        if (xhttp_stream_auto_connect_next_track(self) == ESP_OK) {
            rlen = esp_http_client_read(http->client, buffer, len);
        }
    }
    if (rlen <= 0) {
        ESP_LOGW(TAG, "No more data, errno:%d, total_bytes:%llu, rlen = %d",
                 errno, info.byte_pos, rlen);
        ...
        if (http->auto_connect_next_track) {
            ...
            if (dispatch_hook(self, HTTP_STREAM_FINISH_TRACK, NULL, 0) !=
                ESP_OK) {
                ESP_LOGE(TAG, "Failed to process user callback");
                return ESP_FAIL;
            }
    ...
    ESP_LOGD(TAG, "req lengh=%d, read=%d, pos=%d/%d", len, rlen,
             (int)info.byte_pos, (int)info.total_bytes);
    return rlen;
}

static int _http_write(audio_element_handle_t self, char *buffer, int len,
                       TickType_t ticks_to_wait, void *context) {
    xhttp_stream_t *http = (xhttp_stream_t *)audio_element_getdata(self);
    ...
    if ((wrlen = esp_http_client_write(http->client, buffer, len)) <= 0) {
        ESP_LOGE(TAG, "Failed to write data to http stream, wrlen=%d, errno=%d",
                 wrlen, errno);
    }
    return wrlen;
}

static int _http_process(audio_element_handle_t self, char *in_buffer,
                         int in_len) {
...
audio_element_handle_t xhttp_stream_init(xhttp_stream_cfg_t *config) {
    ...
    xhttp_stream_t *http = audio_calloc(1, sizeof(xhttp_stream_t));
    ...
    if (http->enable_playlist_parser) {
        ...
        http->variant_playlist->data =
            audio_calloc(1, MAX_PLAYLIST_LINE_SIZE + 1);
...
esp_err_t xhttp_stream_next_track(audio_element_handle_t el) {
    xhttp_stream_t *http = (xhttp_stream_t *)audio_element_getdata(el);
    if (!(http->enable_playlist_parser && http->is_playlist_resolved)) {
        /**
...
esp_err_t xhttp_stream_auto_connect_next_track(audio_element_handle_t el) {
    audio_element_info_t info;
